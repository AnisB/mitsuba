...
public:
	...
	Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		Spectrum res = (generateRandomNumber() > 0.5f) ? 
			2.0f * eval_conductor(wi, wo, alpha_x, alpha_y, m_eta, m_k, m_scatteringOrderMax, m_cap) :
			2.0f * eval_conductor(wo, wi, alpha_x, alpha_y, m_eta, m_k, m_scatteringOrderMax, m_cap)/Frame::cosTheta(bRec.wi)*Frame::cosTheta(bRec.wo);
