...
Spectrum eval_diffuse(const vec3& wi, const vec3& wo, const float alpha_x, const float alpha_y, const Spectrum& albedo, const int scatteringOrderMax, const bool cap)
{
	if(wi.z <= 0 || wo.z <= 0)
	...
	while(current_scatteringOrder < scatteringOrderMax)
	{
		...
		// sample VNDF
		vec3 wm = sampleVNDF(-ray.w, alpha_x, alpha_y, cap);
...
vec3 sample_diffuse(const vec3& wi, const float alpha_x, const float alpha_y, const Spectrum& albedo, const int scatteringOrderMax, const bool cap, Spectrum& energy)
{
...
public:
	...
	Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		Spectrum res = (wi.z < wo.z) ?
			eval_diffuse(wi, wo, alpha_x, alpha_y, albedo, m_scatteringOrderMax, m_cap) :
			eval_diffuse(wo, wi, alpha_x, alpha_y, albedo, m_scatteringOrderMax, m_cap)/Frame::cosTheta(bRec.wi)*Frame::cosTheta(bRec.wo);
	...
	Spectrum sample(BSDFSamplingRecord &bRec, Float &pdf, const Point2 &sample) const {
		...
		Spectrum albedo = m_reflectance->eval(bRec.its);
		Spectrum energy;
		vec3 wo = sample_diffuse(wi, alpha_x, alpha_y, albedo, m_scatteringOrderMax, m_cap, energy);
		bRec.wo.x = wo.x;
		bRec.wo.y = wo.y;
		bRec.wo.z = wo.z;
